...
public class DetailsDialog {
    ...
    private static class DetailsAdapter extends BaseAdapter {
            ...
            for (Entry<Integer, Object> detail : details) {
                ...
                switch (detail.getKey()) {
                    ...
                    case MediaDetails.INDEX_EXPOSURE_TIME: {
                        ...
                        if (time < 1.0f) {
                            value = String.format(/*mDefaultLocale,*/Locale.ENGLISH, "%d/%d", 1,
                                    (int) (0.5f + 1 / time));
                        } else {
                ...
                if (details.hasUnit(key)) {
                    value = String.format(Locale.ENGLISH,"%s: %s %s", getDetailsName(
                            context, key), value, context.getString(details.getUnit(key)));
                } else {
                    value = String.format(Locale.ENGLISH,"%s: %s", getDetailsName(
                            context, key), value);
                }
                mItems.add(value);
            }
        ...
        public void onResolutionAvailable(int width, int height) {
            ...
            String widthString = String.format(/*mDefaultLocale,*/Locale.ENGLISH, "%s: %d",
                    getDetailsName(
                            mContext, MediaDetails.INDEX_WIDTH), width);
            String heightString = String.format(/*mDefaultLocale,*/Locale.ENGLISH, "%s: %d",
                    getDetailsName(
        ...
        private String toLocalNumber(int n) {
            return String.format(/*mDefaultLocale,*/Locale.ENGLISH, "%d", n);
        }
