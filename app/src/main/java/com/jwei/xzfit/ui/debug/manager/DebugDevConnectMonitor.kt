package com.jwei.xzfit.ui.debug.manager
...
object DebugDevConnectMonitor {
    ...
    fun devConnectStateChange(state: Int) {
        if (getSwitch()) {
            if (mMonitorFileName.isNotEmpty()) {
                ...
                stringBuilder.append(TimeUtils.getNowString(com.jwei.xzfit.utils.TimeUtils.getSafeDateFormat(com.jwei.xzfit.utils.TimeUtils.DATEFORMAT_COMM)))
                when (state) {
    ...
    private fun createFileName(): String {
        return "Dev_Connect_${TimeUtils.date2String(Date(), com.jwei.xzfit.utils.TimeUtils.getSafeDateFormat("yyyy_MM_dd_HH_mm_ss"))}.txt"
    }
