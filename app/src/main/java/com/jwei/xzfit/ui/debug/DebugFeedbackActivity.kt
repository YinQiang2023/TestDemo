...
import com.jwei.xzfit.databinding.ActivityDebugFeedbackBinding
import com.jwei.xzfit.databinding.DebugDialogFeedbackIllustratingBinding
        ...
        when (v?.id) {
            binding.btnCopy.id -> {
                val info = StringBuilder().apply {
                    append("反馈时间:\n").append(TimeUtils.getNowString(com.jwei.xzfit.utils.TimeUtils.getSafeDateFormat(com.jwei.xzfit.utils.TimeUtils.DATEFORMAT_COMM))).append("\n\n")
            ...
            binding.btnShare.id -> {
                PermissionUtils.checkRequestPermissions(
                    ...
                    val zipFilePath = getExternalFilesDir("logZip")?.absolutePath + File.separator +
                            "Android_log_" + TimeUtils.date2String(Date(), com.jwei.xzfit.utils.TimeUtils.getSafeDateFormat("yyyy-MM-dd")) + ".zip"
        ...
        ThreadUtils.executeByIo(object : ThreadUtils.Task<String>() {
            ...
            override fun doInBackground(): String {
                ...
                for (file in listAllFilesInDir(logDir)) {
                    //只分享今日log
                    if (isToday) {
                        if (file.absolutePath.contains(TimeUtils.date2String(Date(), com.jwei.xzfit.utils.TimeUtils.getSafeDateFormat("yyyy-MM-dd")))) {
