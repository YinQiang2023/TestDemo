package com.jwei.xzfit.ui.device.theme
...
import com.jwei.xzfit.databinding.ActivityDiyDialBinding
...
import com.jwei.xzfit.https.response.DiyDialInfoResponse
import com.jwei.xzfit.ui.adapter.FunctionsAdapter
import com.jwei.xzfit.ui.adapter.StyleAdapter
import com.jwei.xzfit.ui.data.Global
import com.jwei.xzfit.ui.device.bean.BulkDownloadListener
import com.jwei.xzfit.ui.device.bean.StyleBean
import com.jwei.xzfit.ui.device.bean.diydial.MyDiyDialUtils
import com.jwei.xzfit.ui.device.bean.diydial.MyDiyDialUtils.getDiyDialJsonByDiyDialInfoResponse
...
import com.jwei.xzfit.utils.FileUtils
import com.jwei.xzfit.utils.PermissionUtils
            ...
            if (it.code == HttpCommonAttributes.REQUEST_SUCCESS) {
                ...
                viewModel.downloadInfoRes(it.data, object : BulkDownloadListener {
                    ...
                    override fun onFailed(msg: String) {
                        ...
                        com.jwei.xzfit.utils.ToastUtils.showToast(R.string.img_fail_tip)
            ...
            EventAction.ACTION_DEVICE_BLE_STATUS_CHANGE -> {
                when (msg.arg) {
                    BleCommonAttributes.STATE_CONNECTED -> {
                        com.jwei.xzfit.utils.LogUtils.i(TAG, "device connected")
                    }
                    BleCommonAttributes.STATE_CONNECTING -> {
                        com.jwei.xzfit.utils.LogUtils.i(TAG, "device connecting")
                    }
                    BleCommonAttributes.STATE_DISCONNECTED -> {
                        com.jwei.xzfit.utils.LogUtils.e(TAG, "device disconnect")
                        if (isSending) {
                            isSending = false
            ...
            override fun onSuccess(statusValue: Int, statusName: String) {
                ...
                when (statusValue) {
                    ...
                    DeviceWatchFaceFileStatusListener.PrepareStatus.BUSY.state -> {
                        com.jwei.xzfit.utils.ToastUtils.showToast(R.string.ota_device_busy_tips)
                    }
                    DeviceWatchFaceFileStatusListener.PrepareStatus.DUPLICATED.state -> {
                        com.jwei.xzfit.utils.ToastUtils.showToast(R.string.ota_device_is_duplicated)
                    }
                    DeviceWatchFaceFileStatusListener.PrepareStatus.LOW_STORAGE.state -> {
                        com.jwei.xzfit.utils.ToastUtils.showToast(R.string.healthy_sports_sync_fail)
                    }
                    DeviceWatchFaceFileStatusListener.PrepareStatus.LOW_BATTERY.state -> {
                        com.jwei.xzfit.utils.ToastUtils.showToast(R.string.ota_device_low_power_tips)
                    }
                    DeviceWatchFaceFileStatusListener.PrepareStatus.DOWNGRADE.state -> {
                        com.jwei.xzfit.utils.ToastUtils.showToast(R.string.ota_device_timeout_tips)
                    }
                }
                if (!isReady) {
...
//                    strongDialog = null
                    com.jwei.xzfit.utils.LogUtils.i(TAG, "sendCustomDial startUploadBigData onSuccess")
                    com.jwei.xzfit.utils.ToastUtils.showToast(R.string.sync_success_tips)
                    //同步成功后缓存效果图路径
...
//                                                                UnitConversionUtils.bigDecimalFormat((curPiece / dataPackTotalPieceLength.toFloat()) * 100)
                ...
                override fun onTimeout() {
                    if (isSending) {
                        com.jwei.xzfit.utils.LogUtils.e(TAG, "sendCustomDial startUploadBigData onTimeout")
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            RESULT_GET_PHOTOGRAPH_CODE -> {
                if (resultCode == RESULT_OK) {
                    if (data == null || data.data == null) {
                        com.jwei.xzfit.utils.LogUtils.d(TAG, "获取相册异常：data == null")
                    ...
                    try {
                        ...
                        if (destFile == null) {
                            com.jwei.xzfit.utils.LogUtils.d(TAG, "获取相册异常：复制相册图片失败！")
                            photoTrackingLog = TrackingLog.getAppTypeTrack("获取相册图片资源错误").apply {
                                log = "获取相册异常：复制相册图片失败！"
                            }
                            AppTrackingManager.trackingModule(AppTrackingManager.MODULE_SYNC_DIAL, photoTrackingLog!!, "1812", true)
                        ...
                        e.printStackTrace()
                        com.jwei.xzfit.utils.LogUtils.d(TAG, "获取相册异常：$e")
                        photoTrackingLog = TrackingLog.getAppTypeTrack("获取相册图片资源错误").apply {
                            log = "获取相册异常：$e"
                        }
                        AppTrackingManager.trackingModule(AppTrackingManager.MODULE_SYNC_DIAL, photoTrackingLog!!, "1812", true)
            ...
            UCrop.REQUEST_CROP -> {
                ...
                if (resultCode == RESULT_OK) {
                    if (data == null) {
                        com.jwei.xzfit.utils.LogUtils.d(TAG, "裁剪异常：data == null")
                    ...
                    if (data != null) {
                        ...
                        if (cropError != null) {
                            ...
                            com.jwei.xzfit.utils.LogUtils.d(TAG, "裁剪异常：$cropError")
                            cropTrackingLog = TrackingLog.getAppTypeTrack("裁剪图片资源错误").apply {
                                log = "裁剪异常：$cropError"
                            }
                            AppTrackingManager.trackingModule(AppTrackingManager.MODULE_SYNC_DIAL, cropTrackingLog!!, "1813", true)
