...
import com.jwei.xzfit.db.model.sport.ExerciseIndoor
import com.jwei.xzfit.db.model.sport.ExerciseOutdoor
import com.jwei.xzfit.db.model.sport.ExerciseSwimming
...
import com.jwei.xzfit.utils.SendCmdUtils
...
object DevSportManager {
    ...
    private fun saveSportInfo(sportInfo: DevSportInfoBean) {
        LogUtils.e("设备运动数据同步----->")
        com.jwei.xzfit.utils.LogUtils.d("sportInfo", sportInfo.toString())
    ...
    private fun isSportDevSport(): Boolean {
        if (deviceSettingBean != null && !deviceSettingBean!!.functionRelated.auxiliary_exercise) {
            if (!isSaveNoSportLog) {
                com.jwei.xzfit.utils.LogUtils.e("DevSport", "设备不支持辅助运动", true)
                //本次进程生命避免重复存入日志
