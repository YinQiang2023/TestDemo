...
import com.jwei.publicone.databinding.FragmentSportBinding
...
import com.jwei.publicone.ui.sport.SportCountDownActivity
...
class SportFragment : BaseFragment<FragmentSportBinding, SportModel>(
    ...
    override fun initView() {
        ...
        if (PermissionUtils.isGranted(*com.jwei.publicone.utils.PermissionUtils.PERMISSION_GROUP_LOCATION)) {
            isFirstLoadMap = true
            isInitMap = true
            if (!AppUtils.isEnableGoogleMap()) {
                initAMap()
...
//                    viewmodel.getOdometer(mSportType)
    ...
    override fun onVisible() {
        ...
        if (!isInitMap && PermissionUtils.isGranted(*com.jwei.publicone.utils.PermissionUtils.PERMISSION_GROUP_LOCATION)) {
            if (!AppUtils.isEnableGoogleMap()) {
                initAMap()
            } else {
                initGoogleMap()
    ...
    fun showPermissionExplainDialog() {
        DialogUtils.showDialogTwoBtn(
            ...
            object : DialogUtils.DialogClickListener {
                override fun OnOK() {
                    LogUtils.d(com.jwei.publicone.utils.PermissionUtils.TAG, "申请权限")
                    requestPermissions()
                }
    ...
    fun requestPermissions() {
        ...
        com.jwei.publicone.utils.PermissionUtils.requestPermissions(
            this.lifecycle,
            ...
            com.jwei.publicone.utils.PermissionUtils.PERMISSION_GROUP_LOCATION
        ) {
    ...
    fun checkQLocationPermission(): Boolean {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            if (!PermissionUtils.isGranted(*com.jwei.publicone.utils.PermissionUtils.PERMISSION_10_LOCATION)) {
                DialogUtils.dialogShowContentAndTwoBtn(requireActivity(), getString(R.string.background_location_tips),
                    getString(R.string.dialog_cancel_btn), getString(R.string.dialog_confirm_btn), object : DialogUtils.DialogClickListener {
                        override fun OnOK() {
                            PermissionUtils.permission(*com.jwei.publicone.utils.PermissionUtils.PERMISSION_10_LOCATION)
