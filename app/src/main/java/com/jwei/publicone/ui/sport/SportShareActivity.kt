...
import com.jwei.publicone.databinding.ActivitySportShareBinding
import com.jwei.publicone.databinding.ItemSportShareCustomBgBinding
import com.jwei.publicone.databinding.ItemSportShareCustomSytleBinding
...
class SportShareActivity : BaseActivity<ActivitySportShareBinding, SportModel>(
    ...
    override fun initView() {
        ...
        val t = TimeUtils.millis2String(sportModleInfo!!.sportTime * 1000, com.jwei.publicone.utils.TimeUtils.getSafeDateFormat("yyyy/MM/dd HH:mm")).split(" ")
        binding.userLayout.tvDate.text = SpannableStringTool.get()
            ...
            .setForegroundColor(Color.WHITE)
            .create()
        binding.customLayout.tvCustomDate.text = TimeUtils.millis2String(sportModleInfo!!.sportTime * 1000, com.jwei.publicone.utils.TimeUtils.getSafeDateFormat("MM/dd"))
            ...
            RESULT_GET_PHOTOGRAPH_CODE -> {
                if (resultCode == RESULT_OK) {
                    if (data == null || data.data == null) {
                        com.jwei.publicone.utils.LogUtils.d(TAG, "获取相册异常：data == null", true)
                        return
                    }
                    try {
                        ...
                        if (destFile == null) {
                            com.jwei.publicone.utils.LogUtils.d(TAG, "获取相册异常：复制相册图片失败！", true)
                            ToastUtils.showToast(R.string.img_fail_tip)
                            return
                        }
                        //加载图片
                        ...
                        e.printStackTrace()
                        com.jwei.publicone.utils.LogUtils.d(TAG, "获取相册异常：$e", true)
                        ToastUtils.showToast(R.string.img_fail_tip)
                    }
                }
            }
    ...
    private fun addTimeBean(styleBeans: MutableList<StyleBean>, isShowTitle: Boolean = false) {
        styleBeans.add(
            StyleBean(
                ...
                com.jwei.publicone.utils.TimeUtils.millis2String(sportModleInfo!!.sportDuration * 1000L),
                ""
            )
        )
    }
