...
import com.jwei.publicone.db.model.track.AppTrackingLog
import com.jwei.publicone.db.model.track.BehaviorTrackingLog
import com.jwei.publicone.db.model.track.DevTrackingLog
import com.jwei.publicone.db.model.track.TrackingLog
import com.jwei.publicone.https.HttpCommonAttributes
import com.jwei.publicone.https.params.DevTrackingParam
import com.jwei.publicone.https.params.TrackingAppParam
import com.jwei.publicone.https.params.UserBehaviorParam
import com.jwei.publicone.https.tracking.TrackingRetrofitClient
import com.jwei.publicone.ui.data.Global
import com.jwei.publicone.utils.AppUtils
import com.jwei.publicone.utils.HttpLog
...
object AppTrackingManager {
    ...
    fun trackingModule(moduleId: Int, trackingLog: TrackingLog, code: String = "", isEnd: Boolean = false, isStart: Boolean = false) {
        ...
        if (moduleId < 10 || moduleId > 21) {
            com.jwei.publicone.utils.LogUtils.e(TAG, "日志埋点异常 -- moduleId:$moduleId,trackingLog:$trackingLog")
            return
        }
        ...
        scope.launch {
            try {
                    ...
                    litePalMutex.withLock {
                        ...
                        appTrackingLog.apply {
                            ...
                            trackingLog.startTime?.let {
                                startTimestamp = (TimeUtils.string2Millis(it, com.jwei.publicone.utils.TimeUtils.getSafeDateFormat("yyyy-MM-dd HH:mm:ss:SSS")) / 1000).toString()
                            }
                            isUpLoad = false
                        }
                ...
                e.printStackTrace()
                com.jwei.publicone.utils.LogUtils.e(TAG, "记录异常埋点日志失败：$e")
            }
        }
    }
            ...
            try {
                withContext(Dispatchers.IO) {
                    litePalMutex.withLock {
                        ...
                        if (dbMap.isEmpty()) {
                            com.jwei.publicone.utils.LogUtils.d(TAG, "异常埋点日志为空")
                            return@withLock
                        }
                        ...
                        //上报
                        if (data.isEmpty()) {
                            com.jwei.publicone.utils.LogUtils.d(TAG, "异常埋点日志填充服务器数据为空")
                            return@withLock
                        }
                        ...
                        for (serItem in data) {
                            ...
                            com.jwei.publicone.utils.LogUtils.d(TAG, "postTrackingDataToServer:${GsonUtils.toJson(result)}")
                            if (result.code == HttpCommonAttributes.REQUEST_SUCCESS) {
                                updateSuccess(serItem)
                            } else {
                                com.jwei.publicone.utils.LogUtils.e(TAG, "上报异常埋点日志失败：${GsonUtils.toJson(result)}")
                ...
                e.printStackTrace()
                com.jwei.publicone.utils.LogUtils.e(TAG, "上报异常埋点日志失败：$e")
    ...
    private fun updateSuccess(serItem: MutableList<TrackingAppParam>) {
        scope.launch {
            try {
                ...
                e.printStackTrace()
                com.jwei.publicone.utils.LogUtils.e(TAG, "处理上报成功的数据库失败：$e")
            }
        }
    }
            ...
            try {
                withContext(Dispatchers.IO) {
                    litePalMutex.withLock {
                        ...
                        if (dbData.isEmpty()) {
                            com.jwei.publicone.utils.LogUtils.d(TAG, "用户行为日志埋点为空")
                            return@withLock
                        }
                        //清除一周前已上传过的数据
                        ...
                        for (section in segmentations) {
                            ...
                            com.jwei.publicone.utils.LogUtils.d(TAG, "postBehaviorTracking:${GsonUtils.toJson(result)}")
                            if (result.code == HttpCommonAttributes.REQUEST_SUCCESS) {
                                updateBehaviorSuccess(section)
                            } else {
                                com.jwei.publicone.utils.LogUtils.e(TAG, "上报用户行为日志失败：${GsonUtils.toJson(result)}")
                ...
                e.printStackTrace()
                com.jwei.publicone.utils.LogUtils.e(TAG, "用户行为日志上传失败：$e")
            ...
            try {
                withContext(Dispatchers.IO) {
                    litePalMutex.withLock {
                        ...
                        if (dbData.isNullOrEmpty()) {
                            com.jwei.publicone.utils.LogUtils.d(TAG, "设备埋点日志数据为空")
                            return@withLock
                        }
                        //上传至服务器
                        for (item in dbData) {
                            ...
                            com.jwei.publicone.utils.LogUtils.d(TAG, "postDeviceTrackingLog:${GsonUtils.toJson(result)}")
                            if (result.code == HttpCommonAttributes.REQUEST_SUCCESS) {
                                updateDevSuccess(item)
                            } else {
                                com.jwei.publicone.utils.LogUtils.e(TAG, "上报设备埋点日志失败：${GsonUtils.toJson(result)}")
                ...
                e.printStackTrace()
                com.jwei.publicone.utils.LogUtils.e(TAG, "设备异常日志埋点异常：$e")
