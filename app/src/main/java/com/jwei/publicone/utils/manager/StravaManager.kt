package com.jwei.publicone.utils.manager
...
import com.jwei.publicone.https.params.StravaOauthTokenParam
import com.jwei.publicone.https.response.StravaTokenResponse
import com.jwei.publicone.https.strava.StravaRetrofitClient
import com.jwei.publicone.ui.user.StravaWebLoginActivity
...
object StravaManager {
     ...
     * @see com.jwei.publicone.ui.HomeActivity.ACTIVITY_REQUEST_CODE
     * @see com.jwei.publicone.utils.manager.GoogleFitManager.GOOGLE_FIT_PERMISSIONS_REQUEST_CODE
     * */
    ...
    private fun createGpxFile(type: Int, recordGpsTime: String, mapData: String): String {
        ...
        if (!mapData.contains(";") || !mapData.contains(",") || !recordGpsTime.contains(",")) {
            com.jwei.publicone.utils.LogUtils.e(TAG, "mapData is invalid", true)
            return ""
        }
        try {
            ...
            //endregion
            val fileName = "sport_${type}_${TimeUtils.date2String(Date(), com.jwei.publicone.utils.TimeUtils.getSafeDateFormat("yyyy_MM_dd_HHmmss"))}.gpx"
            ...
            if (gpxOutputStream == null) {
                com.jwei.publicone.utils.LogUtils.e(TAG, "gpxOutputStream is null", true)
                return ""
            }
            //定位数据
            ...
            val track = Track()
            track.name = "sport_${type}_${TimeUtils.date2String(Date(), com.jwei.publicone.utils.TimeUtils.getSafeDateFormat("yyyy_MM_dd_HHmmss"))}"
            track.type = "1"
            ...
            com.jwei.publicone.utils.LogUtils.e(TAG, "createGpxFile path:$gpxPath")
            return gpxPath
        } catch (e: Exception) {
            com.jwei.publicone.utils.LogUtils.e(TAG, "createGpxFile err:", true)
            e.printStackTrace()
        }
        return ""
    }
    ...
    fun uploads(model: DevSportInfoBean?) {
        ...
        NetworkUtils.isAvailableAsync { isAvailable ->
            if (!isAvailable) {
                com.jwei.publicone.utils.LogUtils.e(TAG, "uploads sport Failure: network unavailable", true)
    ...
    private fun refreshExpiredToken(refresh_token: String, model: DevSportInfoBean) {
        scope.launch {
            try {
                ...
                if (tokenResponse == null) {
                    com.jwei.publicone.utils.LogUtils.d(TAG, "strava getToken Failure: tokenResponse == null", true)
                ...
                e.printStackTrace()
                com.jwei.publicone.utils.LogUtils.d(TAG, "strava getToken Failure:$e", true)
                //SpUtils.getSPUtilsInstance().put(SpUtils.STRAVA_TOKEN_KEY, "")
                AppTrackingManager.trackingModule(AppTrackingManager.MODULE_SYNC_SPORT, TrackingLog.getAppTypeTrack("Strava token过期并且刷新失败").apply {
    ...
    private fun pushDataToStrava(model: DevSportInfoBean) {
        com.jwei.publicone.utils.LogUtils.d(TAG, "start upload sportInfo to Strava.")
        scope.launch {
            try {
                ...
                if (TextUtils.isEmpty(path)) {
                    com.jwei.publicone.utils.LogUtils.e(TAG, "uploads sport Failure: map data error", true)
                ...
                if (fileBody == null) {
                    com.jwei.publicone.utils.LogUtils.e(TAG, "uploads sport Failure:gpx file asRequestBody null", true)
                    AppTrackingManager.trackingModule(AppTrackingManager.MODULE_SYNC_SPORT, trackingLog.apply {
                ...
                trackingLog.serResJson = com.jwei.publicone.utils.AppUtils.toSimpleJsonString(result ?: "{\"data:\":null}")
                AppTrackingManager.trackingModule(AppTrackingManager.MODULE_SYNC_SPORT, trackingLog)

                com.jwei.publicone.utils.LogUtils.d(TAG, "uploads sport Succeeded:$result")
            } catch (e: Exception) {
                com.jwei.publicone.utils.LogUtils.e(TAG, "uploads sport Failure!$e", true)
                e.printStackTrace()
                AppTrackingManager.trackingModule(AppTrackingManager.MODULE_SYNC_SPORT, TrackingLog.getAppTypeTrack("分享轨迹到Strava失败").apply {
