package com.smartwear.xzfit.ui.healthy.ecg.view;
...
public class ECGView extends View {
    ...
    public ECGView(Context context, AttributeSet attrs,
        ...
        //背景线条y
        _backLineColor = typedArray.getColor(R.styleable.elg_BackLineColor, Color.GREEN);
        _titleColor = typedArray.getColor(R.styleable.elg_TitleColor, Color.RED);
        _pointerLineColor = typedArray.getColor(R.styleable.elg_PointerLineColor, Color.WHITE);
        _titleSize = typedArray.getDimensionPixelSize(R.styleable.elg_TitleSize, 30);
        _XYTextSize = typedArray.getDimensionPixelSize(R.styleable.elg_XYTextSize, 20);
        typedArray.recycle();
        initView();
    }
    ...
    private void initView() {
        ...
        //线的大小
        _PaintLine.setStrokeWidth(2.5f);//线的宽度
    ...
    public void drawWave(Canvas canvas) {
        for (int index = 0; index < _ListPoint.size(); index++) {
            if (_ListPoint.size() == _PointMaxAmount && (index >= _CurP && index < _CurP + _RemovedPointNum)) {
                continue;
            }
            if (index > 0) {
                if (_ListPoint.get(index).get(Y_KEY) < 0 || _ListPoint.get(index).get(Y_KEY) < _TopIndent) {
                    continue;
                }
                canvas.drawLine(_ListPoint.get(index - 1).get(X_KEY),
                        _ListPoint.get(index - 1).get(Y_KEY),
                        _ListPoint.get(index).get(X_KEY), _ListPoint.get(index).get(Y_KEY), _PaintLine);
    ...
    public void setLinePoint(float curY) {
        ...
        _CurX += _XUnitLength;
        // 璁＄畻y鐪熷疄锟�?锟斤拷锟�?
        float number = curY / _EveryOneValue;// 杩欎釜鏁板簲璇ュ崰鐨勬牸瀛愭暟
        if (_Height != 0) {
        ...
        if (_CurP < _PointMaxAmount) {
            try {
                if (_ListPoint.size() == _PointMaxAmount && _ListPoint.get(_CurP) != null) {
