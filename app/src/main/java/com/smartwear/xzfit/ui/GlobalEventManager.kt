...
import com.smartwear.xzfit.databinding.DialogHeadsetBondFailedBinding
import com.smartwear.xzfit.databinding.DialogKeExplainBinding
import com.smartwear.xzfit.db.model.track.TrackingLog
import com.smartwear.xzfit.dialog.DialogUtils
import com.smartwear.xzfit.dialog.DialogUtils.DialogClickListener
...
import com.smartwear.xzfit.https.download.DownloadListener
import com.smartwear.xzfit.https.download.DownloadManager
import com.smartwear.xzfit.https.response.AgpsResponse
import com.smartwear.xzfit.https.response.FirewareUpgradeResponse
import com.smartwear.xzfit.https.response.ProductListResponse
import com.smartwear.xzfit.receiver.SifliReceiver
import com.smartwear.xzfit.ui.data.Global
import com.smartwear.xzfit.ui.device.backgroundpermission.BackgroundPermissionMainActivity
import com.smartwear.xzfit.ui.device.bean.DeviceScanQrCodeBean
import com.smartwear.xzfit.ui.device.bean.DeviceSettingBean
import com.smartwear.xzfit.ui.device.bean.NotifyItem
import com.smartwear.xzfit.ui.device.setting.notify.MsgNotifySetActivity
...
object GlobalEventManager {
    ...
    private fun showDownloadingDialog() {
        if (!isOnlyDeviceType &&
            SpUtils.getSPUtilsInstance().getString(SpUtils.CURRENT_FIRMWARE_PLATFORM, "") != Global.FIRMWARE_PLATFORM_SIFLI &&
            ...
            object : DialogUtils.DialogClickListener {
                override fun OnOK() {
                    NetworkUtils.isAvailableAsync { isAvailable ->
                        ...
                        if (AppUtils.isOpenBluetooth() && (isOnlyDeviceType ||
                                    SpUtils.getSPUtilsInstance().getString(SpUtils.CURRENT_FIRMWARE_PLATFORM, "") == Global.FIRMWARE_PLATFORM_SIFLI ||
                                    ControlBleTools.getInstance().isConnect)
                        ) {
                            showDownloadingDialog()
                ...
                DownloadManager.download(url, listener = object : DownloadListener {
                    ...
                    override fun onSucceed(path: String) {
                        ...
                        SpUtils.setValue(
                            ...
                            TimeUtils.getNowString(com.smartwear.xzfit.utils.TimeUtils.getSafeDateFormat(com.smartwear.xzfit.utils.TimeUtils.DATEFORMAT_COMM))
                        )
                        isAGPSDownloading = false
                        if (path != null) {
                            downloadLog.endTime = TrackingLog.getNowString()
    ...
    private fun showKeepLiveExplanationDialog() {
        topActivity.get()?.let { activity ->
            ...
            if (!com.smartwear.xzfit.BuildConfig.DEBUG) { //debug模式下快速关闭
