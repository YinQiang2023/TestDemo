...
public class HeartRateView extends View {
    ...
    public HeartRateView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray typeArray = context.obtainStyledAttributes(attrs, R.styleable.heartRateView, 0, 0);
        int stepheartRateView_histogram_bg = typeArray.getColor(R.styleable.heartRateView_heartRateView_histogram_bg, getResources().getColor(R.color.transparent));
        int stepheartRateView_histogram = typeArray.getColor(R.styleable.heartRateView_heartRateView_histogram, getResources().getColor(R.color.color_26A9D0));
        int stepheartRateView_histogram_touch = typeArray.getColor(R.styleable.heartRateView_heartRateView_histogram_touch, getResources().getColor(R.color.color_97EEFF));
        int stepheartRateView_x_text_color = typeArray.getColor(R.styleable.heartRateView_heartRateView_x_text_color, getResources().getColor(R.color.color_FFFFFF_50));
        isDrawX0 = typeArray.getBoolean(R.styleable.heartRateView_heartRateView_isDrawX0, false);
        ...
        paintValue.setColor(stepheartRateView_histogram);
        ...
        paint_step_bg.setColor(stepheartRateView_histogram_bg);
        ...
        paint_value_touch.setColor(stepheartRateView_histogram_touch);

        paint_text = new Paint();
        paint_text.setStrokeWidth(dp2px(2));
        paint_text.setAntiAlias(true);
        paint_text.setStyle(Paint.Style.FILL);
        paint_text.setColor(stepheartRateView_x_text_color);
...
//            canvas.drawText("18:00", startX + 216 * (spacing + histogramWidth) - textWidthTime / 2f + histogramWidth / 2f, baseline + textHeight, paint_text);
        ...
        if (type == TODAY) {
            ...
            if (heartRateInfo == null || heartRateInfo.size() == 0) return;
        }
        ...
        if (type == TODAY/*progressTime.length == pointSize*/) {
            ...
            paintValue.setStyle(Paint.Style.FILL);
            for (int i = 0; i < heartRateInfo.size(); i++) {
                float value = heartRateInfo.get(i).progressValue / goal;
                if (value >= 1) {
                    value = 1.0f;
        ...
        if (type == TODAY) {
            ...
            if (heartRateInfo == null || heartRateInfo.size() == 0) return;
            if (dataBlockIndex > heartRateInfo.size() - 1) {
                dataBlockIndex = heartRateInfo.size() - 1;
            }
        }
...
//                    canvas.drawRoundRect(rectF, spacing, spacing, paint_value_touch);
            ...
            float value = heartRateInfo.get(dataBlockIndex).progressValue / goal;
            if (value >= 1) {
                value = 1.0f;
            } else if (value < 0.08 && value > 0) {
                value = 0.05f;
            }
            if (heartRateInfo != null && heartRateInfo.size() > 0) step = heartRateInfo.get(dataBlockIndex).progressValue;
            if (step != 0) {
                RectF rectF = new RectF();
                if (progressTime.length == 7) {
                    if (dataBlockIndex > 0) {
            ...
            data = heartRateInfo.get(dataBlockIndex).progressValue;
            max = heartRateInfo.get(dataBlockIndex).maxValue;
            min = heartRateInfo.get(dataBlockIndex).minValue;
        }
...
//            goal = goal * 1.2f;
    ...
    public void setProgress(List<HeartRateInfo> heartRateInfo, String[] progress_time, int type) {
...
//        pointSize = progress_time.length;
        ...
        goal = 0;
        for (int i = 0; i < heartRateInfo.size(); i++) {
            float v = heartRateInfo.get(i).progressValue;
            if (v > goal) {
                goal = v;
            }
        }
...
//            goal = goal * 1.2f;
    ...
    private List<HeartRateInfo> heartRateInfo;
    ...
    public void testData() {
        ...
        List<HeartRateInfo> heartRateInfo = new ArrayList<>();
        for (int i = 0; i < 7; i++) {
...
//        WEEK
        setProgress(heartRateInfo, progressTime, WEEK);
    }

    public void clearData() {
        if (progressValue != null) progressValue = new float[0];
        if (heartRateInfo != null) heartRateInfo.clear();
