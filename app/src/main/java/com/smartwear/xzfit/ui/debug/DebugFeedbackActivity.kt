...
import com.smartwear.xzfit.databinding.ActivityDebugFeedbackBinding
import com.smartwear.xzfit.databinding.DebugDialogFeedbackIllustratingBinding
        ...
        when (v?.id) {
            binding.btnCopy.id -> {
                val info = StringBuilder().apply {
                    append("反馈时间:\n").append(TimeUtils.getNowString(com.smartwear.xzfit.utils.TimeUtils.getSafeDateFormat(com.smartwear.xzfit.utils.TimeUtils.DATEFORMAT_COMM))).append("\n\n")
        ...
        ThreadUtils.executeByIo(object : ThreadUtils.Task<String>() {
            ...
            override fun doInBackground(): String {
                ...
                for (file in listAllFilesInDir(logDir)) {
                    //只分享今日log
                    if (isToday) {
                        if (file.absolutePath.contains(TimeUtils.date2String(Date(), com.smartwear.xzfit.utils.TimeUtils.getSafeDateFormat("yyyy-MM-dd")))) {
