...
import com.smartwear.xzfit.view.wheelview.contract.OnNumberSelectedListener;
import com.smartwear.xzfit.view.wheelview.contract.OnOptionSelectedListener;
...
public class NumberWheelLayout extends OptionWheelLayout {
    ...
    protected void onAttributeSet(@NonNull Context context, @NonNull TypedArray typedArray) {
        ...
        getRightLabel().setText(typedArray.getString(R.styleable.NumberWheelLayout_wheel_label));
    ...
    public void setRangeStep(int min, int max, int step) {
        int minValue = Math.min(min, max);
        int maxValue = Math.max(min, max);
        // 指定初始容量，避免OutOfMemory
