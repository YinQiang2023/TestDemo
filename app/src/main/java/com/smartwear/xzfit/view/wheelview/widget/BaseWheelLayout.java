...
import com.smartwear.xzfit.view.wheelview.contract.OnWheelChangedListener;
...
public abstract class BaseWheelLayout extends LinearLayout implements OnWheelChangedListener {
    ...
    public void setCyclicEnabled(boolean cyclic, boolean isIgnoreLast) {
        if (isIgnoreLast) {
            for (int i = 0; i < wheelViews.size() - 1; i++) {
                wheelViews.get(i).setCyclicEnabled(cyclic);
            }
        } else {
            for (WheelView wheelView : wheelViews) {
                wheelView.setCyclicEnabled(cyclic);
