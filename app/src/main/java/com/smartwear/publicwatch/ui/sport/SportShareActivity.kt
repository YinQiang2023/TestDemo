...
import com.smartwear.publicwatch.databinding.ActivitySportShareBinding
import com.smartwear.publicwatch.databinding.ItemSportShareCustomBgBinding
import com.smartwear.publicwatch.databinding.ItemSportShareCustomSytleBinding
...
class SportShareActivity : BaseActivity<ActivitySportShareBinding, SportModel>(
    ...
    override fun initView() {
        ...
        val t = TimeUtils.millis2String(sportModleInfo!!.sportTime * 1000, com.smartwear.publicwatch.utils.TimeUtils.getSafeDateFormat("yyyy/MM/dd HH:mm")).split(" ")
        binding.userLayout.tvDate.text = SpannableStringTool.get()
            ...
            .setForegroundColor(Color.WHITE)
            .create()
        binding.customLayout.tvCustomDate.text = TimeUtils.millis2String(sportModleInfo!!.sportTime * 1000, com.smartwear.publicwatch.utils.TimeUtils.getSafeDateFormat("MM/dd"))
            ...
            RESULT_GET_PHOTOGRAPH_CODE -> {
                if (resultCode == RESULT_OK) {
                    if (data == null || data.data == null) {
                        com.smartwear.publicwatch.utils.LogUtils.d(TAG, "获取相册异常：data == null", true)
                        return
                    }
                    try {
                        ...
                        if (destFile == null) {
                            com.smartwear.publicwatch.utils.LogUtils.d(TAG, "获取相册异常：复制相册图片失败！", true)
                            ToastUtils.showToast(R.string.img_fail_tip)
                            return
                        }
                        //加载图片
                        ...
                        e.printStackTrace()
                        com.smartwear.publicwatch.utils.LogUtils.d(TAG, "获取相册异常：$e", true)
                        ToastUtils.showToast(R.string.img_fail_tip)
                    }
                }
            }
    ...
    private fun addTimeBean(styleBeans: MutableList<StyleBean>, isShowTitle: Boolean = false) {
        styleBeans.add(
            StyleBean(
                ...
                com.smartwear.publicwatch.utils.TimeUtils.millis2String(sportModleInfo!!.sportDuration * 1000L),
                ""
            )
        )
    }
