...
class DeviceSportChartView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    init {
        ...
        paintText.color = ContextCompat.getColor(context, R.color.color_FFFFFF)
        ...
        paintStandardLine.color = ContextCompat.getColor(context, R.color.color_FFFFFF_12)
...
//        canvas.drawText((maxY * 4 / 4).toString(), padding.toFloat(), padding + 0 * spaceingY + textHeight * 1 / 3, paintText)
        if (type == 1) {
            var textWidth4 = getTextWidth(paintText, text4)
            ...
            canvas.drawText(text4, startX - textWidth4, padding + 0 * spaceingY + textHeight * 1 / 3, paintText)
            ...
            if (maxY < 30) {
                canvas.drawText(String.format(Locale.ENGLISH, "%.1f", (maxY * 4.0 / 4)), padding.toFloat(), padding + 0 * spaceingY + textHeight * 1 / 3, paintText)
                canvas.drawText(String.format(Locale.ENGLISH, "%.1f", (maxY * 3.0 / 4)), padding.toFloat(), padding + 1 * spaceingY + textHeight * 1 / 3, paintText)
                canvas.drawText(String.format(Locale.ENGLISH, "%.1f", (maxY * 2.0 / 4)), padding.toFloat(), padding + 2 * spaceingY + textHeight * 1 / 3, paintText)
                canvas.drawText(String.format(Locale.ENGLISH, "%.1f", (maxY * 1.0 / 4)), padding.toFloat(), padding + 3 * spaceingY + textHeight * 1 / 3, paintText)
            } else {
                canvas.drawText((maxY * 4 / 4).toInt().toString(), padding.toFloat(), padding + 0 * spaceingY + textHeight * 1 / 3, paintText)
        ...
        var textUnit = "(" + resources.getString(R.string.avg_min) + ")"
        canvas.drawText(textUnit, startX - getTextWidth(paintText, textUnit) - dp2px(4), baseline/* + textHeight*/, paintText)
        canvas.drawText("0.0", startX - getTextWidth(paintText, "0.0") / 2, baseline + textHeight, paintText)
        ...
        for (i in xData.indices) {
            ...
            if (xData.size == 17) {
                if (i < xData.size - 1) {
                    ...
                    if ((i + 1) % 2 == 0) {
                        canvas.drawText(
                            String.format(Locale.ENGLISH, "%.1f", xData[i]),
                            x - getTextWidth(paintText, String.format(Locale.ENGLISH, "%.1f", xData[i])) / 2,
                            baseline + textHeight,
                            paintText
                        )
...
//                    canvas.drawText(String.format(Locale.ENGLISH,"%.1f", xData[i]), x - getTextWidth(paintText, String.format("%.1f", xData[i])) / 2, baseline + textHeight, paintText)
                    canvas.drawLine(x, padding.toFloat(), x, baseline, paintStandardLine)
                }
            } else {
                x = startX + (i + 1) * spacing
...
//        canvas.drawCircle(endp.x.toFloat(), endp.y.toFloat(), dp2px(2).toFloat(), paintCircle)
    ...
    fun setParameter(xData: ArrayList<Double>, yData: ArrayList<Double>) {
        ...
        if (type == 1) { // 配速
            val second4: Int = (maxY * 4 / 4).toInt()
            text4 = String.format(Locale.ENGLISH, "%1$02d'%2$02d\"", (second4 / 60), (second4 % 60))

            val second3: Int = (maxY * 3 / 4).toInt()
            text3 = String.format(Locale.ENGLISH, "%1$02d'%2$02d\"", (second3 / 60), (second3 % 60))

            val second2: Int = (maxY * 2 / 4).toInt()
            text2 = String.format(Locale.ENGLISH, "%1$02d'%2$02d\"", (second2 / 60), (second2 % 60))

            val second1: Int = (maxY * 1 / 4).toInt()
            text1 = String.format(Locale.ENGLISH, "%1$02d'%2$02d\"", (second1 / 60), (second1 % 60))
        }
