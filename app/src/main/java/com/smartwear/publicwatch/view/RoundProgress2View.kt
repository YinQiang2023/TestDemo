...
class RoundProgress2View(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    init {
        val typeArray = context!!.theme.obtainStyledAttributes(attrs, R.styleable.roundProgress2View, 0, 0)
        bgColor = typeArray.getColor(
            R.styleable.roundProgress2View_round_progress_bg, ContextCompat.getColor(context, R.color.color_171717)
        )
        fillingColor = typeArray.getColor(
            R.styleable.roundProgress2View_round_progress_filling, ContextCompat.getColor(context, R.color.app_index_color)//定制
        )
        strokeWidth = typeArray.getColor(
            R.styleable.roundProgress2View_round_progress_width, 10
        )
        ...
        paint.color = ContextCompat.getColor(context!!, R.color.index_bg_color)
...
//        canvas.drawArc(rectF, -90f, 360f * angleProgress3, false, paint)
    ...
    fun setProgress(progress: Float) {
        ...
        this.angleProgress2 = progress
        this.angleProgress3 = 0f

        //处理进度条>0.98f太满
        if (angleProgress2 >= 0.98f && angleProgress2 < 1f) {
            angleProgress2 = 0.98f
        }
//        fillingColor = if (angleProgress2 > 0.5f) {
