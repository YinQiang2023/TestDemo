...
import com.smartwear.publicwatch.db.model.sport.ExerciseIndoor
...
object DevSportManager {
    ...
    private fun saveSportInfo(sportInfo: DevSportInfoBean) {
        LogUtils.e("设备运动数据同步----->")
        com.smartwear.publicwatch.utils.LogUtils.d("sportInfo", sportInfo.toString())
    ...
    private fun isSportDevSport(): Boolean {
        if (deviceSettingBean != null && !deviceSettingBean!!.functionRelated.auxiliary_exercise) {
            if (!isSaveNoSportLog) {
                com.smartwear.publicwatch.utils.LogUtils.e("DevSport", "设备不支持辅助运动", true)
                //本次进程生命避免重复存入日志
