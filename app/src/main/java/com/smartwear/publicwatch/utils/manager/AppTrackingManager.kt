...
import com.smartwear.publicwatch.db.model.track.AppTrackingLog
import com.smartwear.publicwatch.db.model.track.BehaviorTrackingLog
import com.smartwear.publicwatch.db.model.track.DevTrackingLog
import com.smartwear.publicwatch.db.model.track.TrackingLog
import com.smartwear.publicwatch.https.HttpCommonAttributes
import com.smartwear.publicwatch.https.params.DevTrackingParam
import com.smartwear.publicwatch.https.params.TrackingAppParam
import com.smartwear.publicwatch.https.params.UserBehaviorParam
import com.smartwear.publicwatch.https.tracking.TrackingRetrofitClient
import com.smartwear.publicwatch.ui.data.Global
import com.smartwear.publicwatch.utils.AppUtils
import com.smartwear.publicwatch.utils.HttpLog
    ...
    fun trackingModule(moduleId: Int, trackingLog: TrackingLog, code: String = "", isEnd: Boolean = false, isStart: Boolean = false) {
        ...
        scope.launch {
            try {
                    ...
                    litePalMutex.withLock {
                        ...
                        appTrackingLog.apply {
                            ...
                            trackingLog.startTime?.let {
                                startTimestamp = (TimeUtils.string2Millis(it, com.smartwear.publicwatch.utils.TimeUtils.getSafeDateFormat("yyyy-MM-dd HH:mm:ss:SSS")) / 1000).toString()
                            }
                            isUpLoad = false
                        }
                ...
                e.printStackTrace()
                com.smartwear.publicwatch.utils.LogUtils.e(TAG, "记录异常埋点日志失败：$e")
            }
        }
    }
            ...
            try {
                withContext(Dispatchers.IO) {
                    litePalMutex.withLock {
                        ...
                        if (dbMap.isEmpty()) {
                            com.smartwear.publicwatch.utils.LogUtils.d(TAG, "异常埋点日志为空")
                            return@withLock
                        }
                            ...
                            if (dbModuleList != null) {
                                ...
                                for (item in dbModuleList) {
                                    ...
                                    if (item.errorCode.isEmpty() || item.errorCode.endsWith("00")) {
                                        //com.smartwear.publicwatch.utils.LogUtils.d(TAG, "过滤异常日志上报：" + GsonUtils.toJson(item))
                                        item.isUpLoad = true
                                        item.saveUpdate()
                                        continue
                                    }
                        ...
                        //上报
                        if (data.isEmpty()) {
                            com.smartwear.publicwatch.utils.LogUtils.d(TAG, "异常埋点日志填充服务器数据为空")
                            return@withLock
                        }
                        ...
                        for (serItem in data) {
                            ...
                            com.smartwear.publicwatch.utils.LogUtils.d(TAG, "postTrackingDataToServer:${GsonUtils.toJson(result)}")
                            if (result.code == HttpCommonAttributes.REQUEST_SUCCESS) {
                                updateSuccess(serItem)
                            } else {
                                com.smartwear.publicwatch.utils.LogUtils.e(TAG, "上报异常埋点日志失败：${GsonUtils.toJson(result)}")
                ...
                e.printStackTrace()
                com.smartwear.publicwatch.utils.LogUtils.e(TAG, "上报异常埋点日志失败：$e")
    ...
    private fun updateSuccess(serItem: MutableList<TrackingAppParam>) {
        scope.launch {
            try {
                ...
                e.printStackTrace()
                com.smartwear.publicwatch.utils.LogUtils.e(TAG, "处理上报成功的数据库失败：$e")
            }
        }
    }
            ...
            try {
                withContext(Dispatchers.IO) {
                    litePalMutex.withLock {
                        ...
                        if (dbData.isEmpty()) {
                            com.smartwear.publicwatch.utils.LogUtils.d(TAG, "用户行为日志埋点为空")
                            return@withLock
                        }
                        //清除一周前已上传过的数据
                        ...
                        for (section in segmentations) {
                            ...
                            //com.smartwear.publicwatch.utils.LogUtils.d(TAG, "postBehaviorTracking:${GsonUtils.toJson(serItems)}")
                            ...
                            com.smartwear.publicwatch.utils.LogUtils.d(TAG, "postBehaviorTracking:${GsonUtils.toJson(result)}")
                            if (result.code == HttpCommonAttributes.REQUEST_SUCCESS) {
                                updateBehaviorSuccess(section)
                            } else {
                                com.smartwear.publicwatch.utils.LogUtils.e(TAG, "上报用户行为日志失败：${GsonUtils.toJson(result)}")
                ...
                e.printStackTrace()
                com.smartwear.publicwatch.utils.LogUtils.e(TAG, "用户行为日志上传失败：$e")
            ...
            try {
                withContext(Dispatchers.IO) {
                    litePalMutex.withLock {
                        ...
                        if (dbData.isNullOrEmpty()) {
                            com.smartwear.publicwatch.utils.LogUtils.d(TAG, "设备埋点日志数据为空")
                            return@withLock
                        }
                        //上传至服务器
                        for (item in dbData) {
                            ...
                            com.smartwear.publicwatch.utils.LogUtils.d(TAG, "postDeviceTrackingLog:${GsonUtils.toJson(result)}")
                            if (result.code == HttpCommonAttributes.REQUEST_SUCCESS) {
                                updateDevSuccess(item)
                            } else {
                                com.smartwear.publicwatch.utils.LogUtils.e(TAG, "上报设备埋点日志失败：${GsonUtils.toJson(result)}")
                ...
                e.printStackTrace()
                com.smartwear.publicwatch.utils.LogUtils.e(TAG, "设备异常日志埋点异常：$e")
