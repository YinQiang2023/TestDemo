package com.smartwear.publicwatch.expansion
...
internal object RxJavaLifecycle {
...
fun Disposable.disposeOnDestroy(lifeOwner: LifecycleOwner?): Disposable? {
    ...
    var lifecycleObserver = RxJavaLifecycle.getDestroyObserver(lifeOwner.toString())

    if (lifecycleObserver == null) {
        lifecycleObserver = DestroyLifeCycleObserver(lifeOwner)
        RxJavaLifecycle.addDestroyObserver(lifecycleObserver)
    }
...
fun Disposable.disposeOnStop(lifeOwner: LifecycleOwner?): Disposable? {
    ...
    var lifecycleObserver = RxJavaLifecycle.getStopObserver(lifeOwner.toString())

    if (lifecycleObserver == null) {
        lifecycleObserver = StopLifeCycleObserver(lifeOwner)
        RxJavaLifecycle.addStopObserver(lifecycleObserver)
    }
